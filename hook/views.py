...
BASE_URL = "https://devmetashot.pythonanywhere.com"
...
def handle_incoming_message(message_data):
    ...
    try:
        ...
        if text_body[0] == '/':
            ...
            if text_body == '/reset':
            ...
            elif text_body[:4] == '/doc':
                prompt = f"Create a document on {text_body[5:]}"
                ...
                with open(f"temp/doc_{sender_phone_number}.docx", "w") as f:
                    f.write(response['content'])
                
                messenger.send_document(
                    f"{BASE_URL}/temp/doc_{sender_phone_number}.docx",
                    sender_phone_number,
                    caption="Document generated by ALL.AI"
                )
                time.sleep(5)
                os.remove(f"temp/doc_{sender_phone_number}.docx")
                return HttpResponse({'status': 'success'})
